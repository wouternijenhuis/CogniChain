...
public class StreamingHandlerExample(ChatClient chatClient) : IExample
{
    ...
    public async Task RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        var llmResponse = await GetLLMResponseAsync("List 3 Azure services in brief.", cancellationToken);
        ...
        var completeResponse = await handler.ProcessStreamAsync(
            SimulateAzureStreamAsync("Azure Functions is serverless compute.", cancellationToken),
    ...
    private static async IAsyncEnumerable<string> SimulateAzureStreamAsync(
        string content,
